...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install TrashCordInstaller")
	var updateFlag = flag.Bool("repair", false, "Repair TrashCordInstaller")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall TrashCordInstaller")
	...
	if *versionFlag {
		fmt.Println("TrashCordInstaller Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update TrashCordInstaller' option to update, or run with --update-self")
			}
		}()

		choices := []string{
			"Install TrashCordInstaller",
			"Repair TrashCordInstaller",
			"Uninstall TrashCordInstaller",
			"Install OpenAsar",
			"Uninstall OpenAsar",
			"View Help Menu",
			"Update TrashCordInstaller",
			"Quit",
		}
		...
		case "Quit":
			return
		case "Update TrashCordInstaller":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest TrashCordInstaller files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, TrashCordInstaller will likely not work.")
}
